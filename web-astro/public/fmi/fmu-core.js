...
export async function loadComponent(component, { files, onLog, shimBase = './vendor/preview2-shim', aot = null }) {
  let gen, cores, source;
  if (aot) {
    // The transpile already happened, at build time, by running this same
    // vendored js-component-bindgen under Node (tools/wasm/transpile.mjs). Fetch
    // its output instead of shipping the 9.3 MB transpiler and spending ~1.4 s
    // of the reader's CPU on work whose answer is known. Everything downstream
    // -- the wrapper patching, the imports, compile, instantiate -- is the same
    // code on the same bytes.
    const at = (path) => new URL(path, aot.base).href;
    const bytes = async (path) => {
      const r = await fetch(at(path));
      if (!r.ok) throw new Error(`cannot load ${at(path)}: ${r.status} ${r.statusText}`);
      return new Uint8Array(await r.arrayBuffer());
    };
    // jco names its core modules positionally and the generated glue asks for
    // them by that name, so the manifest maps name -> content-addressed blob.
    const entries = await Promise.all(
      Object.entries(aot.manifest.cores).map(async ([n, path]) => [n, await bytes(path)]));
    cores = new Map(entries);
    const glue = await fetch(at(aot.manifest.glue));
    if (!glue.ok) throw new Error(`cannot load ${at(aot.manifest.glue)}: ${glue.status}`);
    source = await glue.text();
    // interfacesOf() reads exports and the native-import filter reads imports;
    // without them the load fails inside this function, not at the fetch.
    gen = { imports: aot.manifest.imports, exports: aot.manifest.exports };
  } else {
    const { generate } = await loadBindgen();
    gen = generate(new Uint8Array(component), {
    ...
    cores = new Map(gen.files.filter(([n]) => n.endsWith('.wasm')));
    source = new TextDecoder().decode(gen.files.find(([n]) => n.endsWith('.js'))[1]);
  }
