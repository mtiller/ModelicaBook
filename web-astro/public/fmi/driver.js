...
export class Driver {
  ...
  async load(archive) {
    ...
    // With an AOT manifest the component binary is never needed: it exists only
    // to be transpiled, and that already happened. Skipping it is what lets the
    // archive be a model description alone -- kilobytes instead of the ~710 KB
    // an FMU carrying its own copy of the runtime costs.
    let component = null;
    if (!this.aot) {
      if (!this.exports.om_fmi_select_component(kind)) throw new Error(this.lastError());
      component = this.binary();
    }
    ...
    this.component = await loadComponent(component, { files, onLog: this.onLog, aot: this.aot });
