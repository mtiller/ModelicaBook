...
export class Session {
  ...
  load(archive, aot = null) {
    const flag = this.cancelFlag ? this.cancelFlag.buffer : null;
    return this.send('load', { archive, cancelFlag: flag, aot }, [archive]);
  }
  ...
  //
  // The payload cannot use `kind`: send() spreads it over the message, whose
  // own `kind` is the discriminator the worker switches on.
  warm(iface) {
    return this.send('warm', { iface });
  }
