...
extending from.  We'll talk more about the ``extends`` clause later when we
...
``omega1_measured`` is set only when the ``when`` statement becomes
...
wheel encoder includes a gear on the rotating shaft.  On either side of
...
Knowing that these events occur whenever the shaft has rotated by
...
.. topic:: Use of the ``pre`` operator
    ...
    conditional expression associated with the ``when`` statement (as we
    have done in the previous example).  This makes it clear that you
    are responding to what was happening before the ``when`` statement
    was triggered.
...
Building on the previous examples in this section, the following
...
try to formulate everything under a single ``when`` statement, like this:
...
can address this situation by placing all the ``when`` statements in an
``algorithm`` section.
